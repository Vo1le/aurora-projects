...
public class PrintempsdeMillenusBiome extends AuroraprojectsModElements.ModElement {
	public static Biome biome;

	public PrintempsdeMillenusBiome(AuroraprojectsModElements instance) {
		super(instance, 165);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(4159204).setWaterFogColor(329011)
						.withSkyColor(7972607).withFoliageColor(-4110966).withGrassColor(-11092678).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.FOREST).depth(0.1f).scale(0.2f)
						.temperature(0.7f).downfall(0.8f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("auroraprojects:printempsde_millenus"));
			}
		}
	}
