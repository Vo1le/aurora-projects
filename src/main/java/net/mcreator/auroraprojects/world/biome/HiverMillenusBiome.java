...
public class HiverMillenusBiome extends AuroraprojectsModElements.ModElement {
	public static Biome biome;

	public HiverMillenusBiome(AuroraprojectsModElements instance) {
		super(instance, 148);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.SNOW).category(Biome.Category.ICY).depth(0.1f).scale(0.2f).temperature(-0.3f)
						.downfall(1f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("auroraprojects:hiver_millenus"));
			}
		}
	}
