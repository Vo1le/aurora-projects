...
public class MalgorkDimension extends AuroraprojectsModElements.ModElement {
	public MalgorkDimension(AuroraprojectsModElements instance) {
		super(instance, 62);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(TerreVolcaniqueBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("auroraprojects:malgork_plaines")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("auroraprojects:malgork_plaines")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("auroraprojects:volcan")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("auroraprojects:volcan")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.219607843137, 0.21568627451, 0.211764705882);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("auroraprojects:malgork"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
